...
import { cancelPosition, cancelProject, claimPosition, claimProjectLeader, getProjectDetail, getProjectReviews, removeProjectMember } from '@/api/project';
...
const fetchProjectDetail = async () => {
  ...
  try { project.value = await getProjectDetail(projectId.value); clockTick.value = Date.now(); reviewHistory.value = (isLeader.value || isAdmin.value) ? await getProjectReviews(projectId.value) : []; }
  catch { project.value = null; reviewHistory.value = []; }
...
const viewLessonPlan = () => { const lesson = project.value?.lesson_plan; if (!lesson) return; uni.showModal({ title: '教案查阅', content: `标题：${lesson.title || project.value.title}\n文件：${lesson.file_name || '本地测试教案.pdf'}\n摘要：${lesson.content || '暂无摘要'}`, showCancel: false, confirmText: '知道了' }); };
